...
export function useCardDeck () {
  ...
  const getScore = () => cards.value.reduce((sum, card) => sum + card.score, 0);
  ...
  const takeCard = (card: Card) => { cards.value.push(card); }
...
function fillDeck () {
  ...
  return suits.reduce<Card[]>((cards, suit) => {
    names.forEach(name => { cards.push({ name, suit, score: cardScores[name] }) });
    return cards;
  }, []);
}
