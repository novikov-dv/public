<template>
  <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" class="button" @click="toggle()">
    <defs>
    ...
    <g fill="url(#linear-gradient)">
      ...
      <path v-if="isDisabled" d="M7,10L96,99L99,96L10,7z"/>
    </g>
  </svg>
</template>
...
export default Vue.extend({
  ...
  computed: {
    isDisabled () {
      return this.$store.state.isMusicDisabled;
    }
  },
  methods: {
    toggle() {
      this.$store.dispatch('toggleMusicDisabled');
    }
  }
});
</script>
...
<style scoped>
  .button {
    ...
    width: 4rem;
    height: 4rem;
    filter: drop-shadow(.2rem .2rem .2rem rgba(0, 0, 0, .6));
