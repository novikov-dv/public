...
export function loadGifSprite (url: string): Promise<AnimatedGIF | undefined> {
  // eslint-disable-next-line @typescript-eslint/no-empty-function
  let resolveOuter: (value: AnimatedGIF | undefined) => void = () => { };
  const result = new Promise<AnimatedGIF | undefined>((resolve) => {
  ...
  loader.load((_, resources) => {
    resolveOuter(resources.image.animation);
  });
