...
export function usePixi(
  ...
  const { blobs, deckSprite } = store.state;
  ...
  onMounted(async () => {
    ...
    const bgTexture = await Texture.fromURL(blobs[CARDS_TABLE_TEXTURE_URL]);
    ...
    cardsTexture = await Texture.fromURL(blobs[CARDS_TEXTURE_URL]);
  ...
  const processAction: CardActionHandler = ({ type, card, cardsCount }: CardActionHandlerArgs): void => {
    switch (type) {
      ...
      case CardActionType.RESET: {
        if (!application || !calculator || !deckSprite)
          return;
        ...
        const {
          ...
          progressReset,
          endReset
        } = createResetCardFunctions(application, calculator, deckSprite);
...
function createMoveCardFunctions (
  ...
  const { x: xTo, y: yTo, width, height, radius } = calculator.getCardPosition(isDealer ? PlacementRole.DEALER : PlacementRole.USER, cardsCount);
